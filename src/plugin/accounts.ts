...
export function generateAccountId(): string { return randomBytes(16).toString('hex'); }
...
export class AccountManager {
  ...
  constructor(accounts: ManagedAccount[], usage: Record<string, UsageMetadata>, strategy: AccountSelectionStrategy = 'sticky') {
    ...
    for (const a of this.accounts) {
      ...
      if (m) { a.usedCount = m.usedCount; a.limitCount = m.limitCount; a.realEmail = m.realEmail; }
    }
  }
    ...
    const available = this.accounts.filter(a => {
      if (!a.isHealthy) {
        if (a.recoveryTime && now >= a.recoveryTime) {
          a.isHealthy = true; delete a.unhealthyReason; delete a.recoveryTime;
          return true;
        }
        return false;
      }
  ...
  updateUsage(id: string, meta: { usedCount: number; limitCount: number; realEmail?: string }): void {
    ...
    if (a) { a.usedCount = meta.usedCount; a.limitCount = meta.limitCount; if (meta.realEmail) a.realEmail = meta.realEmail; }
    this.usage[id] = { ...meta, lastSync: Date.now() };
  }

  addAccount(a: ManagedAccount): void {
    const i = this.accounts.findIndex(x => x.id === a.id);
    if (i === -1) this.accounts.push(a); else this.accounts[i] = a;
  }
  ...
  updateFromAuth(a: ManagedAccount, auth: KiroAuthDetails): void {
    ...
    if (acc) {
      acc.accessToken = auth.access; acc.expiresAt = auth.expires; acc.lastUsed = Date.now();
  ...
  markUnhealthy(a: ManagedAccount, reason: string, recovery?: number): void {
    const acc = this.accounts.find(x => x.id === a.id);
    if (acc) { acc.isHealthy = false; acc.unhealthyReason = reason; acc.recoveryTime = recovery; }
  }
  ...
  toAuthDetails(a: ManagedAccount): KiroAuthDetails {
    ...
    return { refresh: encodeRefreshToken(p), access: a.accessToken, expires: a.expiresAt, authMethod: a.authMethod, region: a.region, profileArn: a.profileArn, clientId: a.clientId, clientSecret: a.clientSecret, email: a.email };
  }
}
