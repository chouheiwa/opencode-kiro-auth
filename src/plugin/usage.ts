  ...
  try {
    ...
    if (Array.isArray(data.usageBreakdownList)) {
      for (const s of data.usageBreakdownList) {
        if (s.freeTrialInfo) { usedCount += s.freeTrialInfo.currentUsage || 0; limitCount += s.freeTrialInfo.usageLimit || 0; }
        usedCount += s.currentUsage || 0; limitCount += s.usageLimit || 0;
      }
    }
...
export function updateAccountQuota(account: ManagedAccount, usage: any, accountManager?: any): void {
  ...
  account.usedCount = meta.usedCount; account.limitCount = meta.limitCount;
