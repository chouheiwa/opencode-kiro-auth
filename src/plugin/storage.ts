...
export function getStoragePath(): string { return join(getBaseDir(), 'kiro-accounts.json'); }
export function getUsagePath(): string { return join(getBaseDir(), 'kiro-usage.json'); }

async function withLock<T>(path: string, fn: () => Promise<T>): Promise<T> {
  await fs.mkdir(dirname(path), { recursive: true });
  try { await fs.access(path); } catch { await fs.writeFile(path, '{}'); }
