...
export function transformToCodeWhisperer(url: string, body: any, model: string, auth: KiroAuthDetails, think = false, budget = 20000): PreparedRequest {
  ...
  let curContent = ''; const curTrs: any[] = [], curImgs: any[] = [];
  if (curMsg.role === 'assistant') {
    const arm: any = { content: '' }; let th = '';
    if (Array.isArray(curMsg.content)) {
      for (const p of curMsg.content) {
        if (p.type === 'text') arm.content += p.text || '';
        else if (p.type === 'thinking') th += (p.thinking || p.text || '');
        else if (p.type === 'tool_use') { if (!arm.toolUses) arm.toolUses = []; arm.toolUses.push({ input: p.input, name: p.name, toolUseId: p.id }); }
      }
    } else arm.content = getContentText(curMsg);
    if (th) arm.content = arm.content ? `<thinking>${th}</thinking>\n\n${arm.content}` : `<thinking>${th}</thinking>`;
    history.push({ assistantResponseMessage: arm }); curContent = 'Continue';
  } else {
    const prev = history[history.length - 1];
  ...
  const request: CodeWhispererRequest = { conversationState: { chatTriggerType: KIRO_CONSTANTS.CHAT_TRIGGER_TYPE_MANUAL, conversationId: convId, history, currentMessage: { userInputMessage: { content: curContent, modelId: resolved, origin: KIRO_CONSTANTS.ORIGIN_AI_EDITOR } } } };
  ...
  const machineId = crypto.createHash('sha256').update(auth.profileArn || auth.clientId || "KIRO_DEFAULT_MACHINE").digest('hex');
  ...
  return { url: KIRO_CONSTANTS.BASE_URL.replace('{{region}}', auth.region), init: { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'Authorization': `Bearer ${auth.access}`, 'amz-sdk-invocation-id': crypto.randomUUID(), 'amz-sdk-request': 'attempt=1; max=1', 'x-amzn-kiro-agent-mode': 'vibe', 'x-amz-user-agent': `aws-sdk-js/1.0.0 KiroIDE-${kiroV}-${machineId}`, 'user-agent': ua, 'Connection': 'close' }, body: JSON.stringify(request) }, streaming: true, effectiveModel: resolved, conversationId: convId };
}
...
export function convertToolsToCodeWhisperer(tools: any[]): any[] {
  return tools.filter(t => !['web_search', 'websearch'].includes((t.name || t.function?.name || '').toLowerCase())).map(t => ({ toolSpecification: { name: t.name || t.function?.name, description: (t.description || t.function?.description || '').substring(0, 9216), inputSchema: { json: t.input_schema || t.function?.parameters || {} } } }));
}
...
function deduplicateToolResults(trs: any[]): any[] {
  const u: any[] = [], s = new Set();
  for (const t of trs) { if (!s.has(t.toolUseId)) { s.add(t.toolUseId); u.push(t); } }
  return u;
}
