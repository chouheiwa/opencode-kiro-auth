...
export async function refreshAccessToken(auth: KiroAuthDetails): Promise<KiroAuthDetails> {
  ...
  if (!res.ok) {
    ...
    let data: any = {}; try { data = JSON.parse(txt); } catch { data = { message: txt }; }
  ...
  return { refresh: encodeRefreshToken(upP), access: acc, expires: Date.now() + ((d.expires_in || 3600) * 1000), authMethod: 'idc', region: auth.region, clientId: auth.clientId, clientSecret: auth.clientSecret, email: auth.email };
}
